...
async function anonUpload(buff, opt = {}) {
    ...
    form.append("file", buff, "xyvnz." + ext)
    try {
      ...
      case "videoRenderer": 
        results.push({
          ...
          download: async function download() {
           return (await  savefrom("https://youtu.be/" + c[type].videoId)).url[0].url
          }
        })
      break;
      case "radioRenderer":
...
export function smeme(atas, bawah, img) {
  ...
  atas = atas.trim().replace(/\s/g, '_').replace(/\?/g, '~q').replace(/\%/g, '~p').replace(/\#/g, '~h').replace(/\//g, '~s')
   ...
   return `https://api-mmgen.herokuapp.com/images/custom/${atas}/${bawah}.png?background=${img}`
}
