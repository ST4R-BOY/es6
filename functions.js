...
export function bindSock(sock) {
  Object.defineProperties(sock, {
    ...
    getName: {
      async value(jid) {
        let name
        if (jid.endsWith("@g.us")) name = (await sock.groupMetadata(jid)).subject
        else if (jid == "0@s.whatsapp.net") name = "WhatsApp"
        else name = store.chats.all().find(v => v.id == jid)?.name || sock.chats[jid]?.notify || PN(`+${jid.split("@")[0]}`).getNumber("international")
        return name
      }
    },
  })
